...
Data downloaded from Qualtrics as a .csv. Some minor cleaning (removing incompleted responses) has already been done in excel, as well as removing the identifiers since I'm sharing this data as a part of the project. Code for that removal is also incldued later.
...
Code to remove identifying information from the dataset.
...
This code does not leave anyone NA, so there is no need to add that line.
...
I know that I have no missing responses, but it's good to have that there for future use. And, it doesn't change anything because there's no missing.
...
Similar to above, I need to make a composite of two variables: "cdiscl_private", "cdiscl_self_talk"
...
Composite made of two variables, "cdiscl_prompt_prof", "cdiscl_prompt_ecour"
...
Composite made of two variables, "gdiscl_private", "gdiscl_self_talk". This is the first composite to have missing variables, and it has a lot of them, because a large portion of our sample did not use groups. This will be discussed in portfolio 2.
...
Composite made of two variables, "gdiscl_prompt_prof", "gdiscl_prompt_encour". Also has missing variables.
